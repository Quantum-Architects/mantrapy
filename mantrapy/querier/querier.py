...
from mantrapy.types.account import Account
from mantrapy.types.account import AccountResponse
from mantrapy.types.account import PubKey
from mantrapy.types.bank import QueryAllBalancesResponse
from mantrapy.types.block import Block
from mantrapy.types.block import BlockID
from mantrapy.types.block import ResultBlock
...
class Querier:
    ...
    def get_account(self, address: str) -> AccountResponse:
        url = self.create_api_url(
            QUERY_PATHS['account'].format(address=address), )
    ...
    def get_balances(self, address: str) -> QueryAllBalancesResponse:
        url = self.create_api_url(
            QUERY_PATHS['balances'].format(address=address), )
        resp = requests.get(url, timeout=TIMEOUT)
    ...
    def get_block_by_hash(self, _hash: str) -> ResultBlock:
        url = self.create_rpc_url(
            QUERY_PATHS['block_by_hash'].format(hash=_hash), )
        resp = requests.get(url, timeout=TIMEOUT)
